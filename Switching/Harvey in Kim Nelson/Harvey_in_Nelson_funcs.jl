...
function simulate_data(params, n_order, n_obs, alpha_init)
    ...
    if S[1] == 1  #Recesion
        ...
        v[1] = rand(Normal(0, sqrt(sigma_epsilon2_0)))
        ...
        v[1] = rand(Normal(0, sqrt(sigma_epsilon2_1))) # sigma_epsilon
        v[2] = rand(Normal(0, sqrt(sigma_xi2)))  # sigma_xi2
    ...
    for t in 2:n_obs
        ...
        if S[t] == 1 #Recesion
            ...
            v[4] = rand(Normal(0, sqrt(params[5]))) #sigma kappa
            v = v + nu1
        else
            # v = rand(MvNormal(zeros(state_dim), Q_0)) + nu0
            ...
            v[4] = rand(Normal(0, sqrt(params[4]))) #sigma kappa
            v = v + nu0
...
function transform_params(params_unbounded, priors)
    ...
    exp_gamma_xi = exp(gamma_sigma_xi2)
    sigma_xi2 = (a_xi + b_xi * exp_gamma_xi) / (1 + exp_gamma_xi)

    exp_gamma_kappa_0 = exp(gamma_sigma_kappa2_0)
    sigma_kappa2_0 = (a_kappa_0 + b_kappa_0 * exp_gamma_kappa_0) / (1 + exp_gamma_kappa_0)

    exp_gamma_kappa_1 = exp(gamma_sigma_kappa2_1)
    sigma_kappa2_1 = (a_kappa_1 + b_kappa_1 * exp_gamma_kappa_1) / (1 + exp_gamma_kappa_1)

    exp_gamma_epsilon_0 = exp(gamma_sigma_epsilon2_0)
    sigma_epsilon2_0 = (a_epsilon_0 + b_epsilon_0 * exp_gamma_epsilon_0) / (1 + exp_gamma_epsilon_0)

    exp_gamma_epsilon_1 = exp(gamma_sigma_epsilon2_1)
    sigma_epsilon2_1 = (a_epsilon_1 + b_epsilon_1 * exp_gamma_epsilon_1) / (1 + exp_gamma_epsilon_1)
...
function log_derivatives_params(params_unbounded, priors)
    ...
    # # Log derivatives for variance parameters (inverse gamma)
    ...
    # Log derivatives for variance parameters (uniform)
    log_derivative_sigma_xi2 = log(b_xi - a_xi) + gamma_sigma_xi2 - 2 * log(1 + exp(gamma_sigma_xi2))
    log_derivative_sigma_kappa2_0 = log(b_kappa_0 - a_kappa_0) + gamma_sigma_kappa2_0 - 2 * log(1 + exp(gamma_sigma_kappa2_0))
    log_derivative_sigma_kappa2_1 = log(b_kappa_1 - a_kappa_1) + gamma_sigma_kappa2_1 - 2 * log(1 + exp(gamma_sigma_kappa2_1))
    log_derivative_sigma_epsilon2_0 = log(b_epsilon_0 - a_epsilon_0) + gamma_sigma_epsilon2_0 - 2 * log(1 + exp(gamma_sigma_epsilon2_0))
    log_derivative_sigma_epsilon2_1 = log(b_epsilon_1 - a_epsilon_1) + gamma_sigma_epsilon2_1 - 2 * log(1 + exp(gamma_sigma_epsilon2_1))
...
function log_prior(theta, priors)
    ...
    log_prior_sigma_kappa2_0 = log(1 / (b_kappa_0 - a_kappa_0))
    log_prior_sigma_kappa2_1 = log(1 / (b_kappa_1 - a_kappa_1))
    log_prior_sigma_epsilon2_0 = log(1 / (b_epsilon_0 - a_epsilon_0))
    log_prior_sigma_epsilon2_1 = log(1 / (b_epsilon_1 - a_epsilon_1))
