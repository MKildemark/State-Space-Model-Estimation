...
function kalman_filter(y, a1, P1, params, n_order)
    ...
    for t in 1:n
        ...
        y_pred = dot(Z, a_pred[t, :])

        # Prediction error (innovation): 
        v[t] = y[t] - y_pred

        # Prediction error variance: 
        F[t] = dot(Z, P_pred[t, :, :] * Z) + H
...
function transform_params(params_unbounded; a_rho=0.001, b_rho=0.970, a_lambda=0.001, b_lambda=π)
...
function log_derivatives_params(params_unbounded; a_rho=0.001, b_rho=0.970, a_lambda=0.001, b_lambda=π)
    gamma_rho, gamma_lambda_c, gamma_sigma_xi2, gamma_sigma_kappa2, gamma_sigma_epsilon2 = params_unbounded
...
function log_prior(theta)
    ...
    log_prior_sigma_kappa2 = a_ig * log(b_ig) - lgamma(a_ig) - (a_ig + 1) * log(sigma_kappa2) - b_ig / sigma_kappa2
    log_prior_sigma_epsilon2 = a_ig * log(b_ig) - lgamma(a_ig) - (a_ig + 1) * log(sigma_epsilon2) - b_ig / sigma_epsilon2

    return log_prior_lambda_c + log_prior_rho + log_prior_sigma_xi2 + log_prior_sigma_kappa2 + log_prior_sigma_epsilon2
end

function log_posterior(gamma, y, a1, P1, n_order)
...
function initialize_mcmc(y, a1, P1, n_order; n_init=40000, burn_init=5000, omega_init=0.1)
...
function recursion_mcmc(y, a1, P1, n_order, chain_init_burned, Sigma; n_rec=20000, burn_rec=10000, omega_rec=0.1)
    dim = 5  # Number of parameters
